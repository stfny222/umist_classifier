...
__all__ = ["explore_full_dataset", "explore_split_datasets"]
...
def explore_split_datasets(
    ...
    display_images(
        ...
        n_samples=10,
        n_subjects=5,
        title="UMIST Dataset - Sample Face Images",
        save=False,
    )
    ...
    if augmented:
        print("(Note: Training counts shown are AFTER augmentation)")
    print("-" * 70)
    ...
    if augmented:
        print(f"  (Each subject augmented equally, maintaining class balance)")
    ...
    bars1 = ax.bar(
        ...
        train_vals,
        width,
        label=f"Train ({train_split*100:.0f}%)",
        color="steelblue",
        alpha=0.8,
        edgecolor="black",
    )
    bars2 = ax.bar(
        ...
        val_vals,
        width,
        label=f"Validation ({val_split*100:.0f}%)",
        color="orange",
        alpha=0.8,
        edgecolor="black",
    )
    bars3 = ax.bar(
        ...
        test_vals,
        width,
        label=f"Test ({test_split*100:.0f}%)",
        color="green",
        alpha=0.8,
        edgecolor="black",
    )
    ...
    print(f"\nActual split ratios:")
    print(f"  Train: {len(y_train)/total*100:.2f}% (target: {train_split*100:.0f}%)")
    print(f"  Validation: {len(y_val)/total*100:.2f}% (target: {val_split*100:.0f}%)")
    print(f"  Test: {len(y_test)/total*100:.2f}% (target: {test_split*100:.0f}%)")
...
if __name__ == "__main__":
    ...
    # Option 2: 30/35/35 split with 7x augmentation
    # explore_split_datasets(augmented=True, augmentation_factor=7, train_split=0.30, val_split=0.35, test_split=0.35)
