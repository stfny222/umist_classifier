"""
Shared clustering evaluation metrics.

These are generic metrics that work with any clustering algorithm.
"""
...
def evaluate_dimensionality_reduction(X_original, X_reduced, n_neighbors=10, 
    ...
    metrics = {
        ...
        "n_components": X_reduced.shape[1] if X_reduced.ndim > 1 else 1,
    }
    
    # Only compute reconstruction error if PCA model is provided
    if pca_model is not None:
        metrics["reconstruction_error"] = compute_reconstruction_error(X_original, X_reduced, pca_model)
        metrics["relative_recon_error"] = compute_relative_reconstruction_error(X_original, X_reduced, pca_model)
    else:
...
def print_dimred_metrics(metrics):
    ...
    if metrics.get('reconstruction_error') is not None:
        print(f"  Reconstruction Error: {metrics['reconstruction_error']:.4f}")
        print(f"  Relative Recon Error: {metrics['relative_recon_error']*100:.2f}%")
    else:
        print(f"  Reconstruction Error: N/A (non-invertible method)")
    print(f"  N Components: {metrics['n_components']}")
